...
############ from .monitorATE import DFIVMonitor
class DFIVMonitor:
    ...
    def record(self, logfile: str, _logfile: str, verbose: int):
        ...
        with torch.no_grad():
            ...
            if verbose >= 1:
                print(f"stage1_insample:{stage1_insample.item()}")
                print(f"stage1_outsample:{stage1_outsample.item()}")
                print(f"stage2_insample:{stage2_insample.item()}")
                print(f"stage2_outsample:{stage2_outsample.item()}")
            ...
            if self.t_loss == 'bin':
                ...
                if stage1_outsample.item() < self.t_eval:
                    print(f"t_eval from {self.t_eval} to {stage1_outsample.item()}. updata t_best~~~~")
                ...
                if stage2_outsample.item() < self.y_eval:
                    print(f"y_eval from {self.y_eval} to {stage2_outsample.item()}. updata y_best~~~~")
                    self.y_eval = stage2_outsample.item()
                    self.y_best = [[train_ate_iv, test_ate_iv], [train_ate_t, test_ate_t]]

                print(f"train_ate_iv, test_ate_iv, train_ate_t, test_ate_t: {train_ate_iv}, {test_ate_iv}, {train_ate_t}, {test_ate_t}")
            #####################################################
            else:
                self.train_y_pred = [insample_2nd_pred, self.train_2nd_data_t.outcome]
                self.val_y_pred = [outsample_2nd_pred, self.validation_data_t.outcome]
...
class DFIVModel:
    ...
    def predict(self, treatment: np.ndarray, covariate: Optional[np.ndarray], device):
        treatment_t = torch.tensor(treatment, dtype=torch.float32)
        covariate_t = None
        if covariate is not None:
            covariate_t = torch.tensor(covariate, dtype=torch.float32)

        treatment_t = treatment_t.to(device)
        covariate_t = covariate_t.to(device)
        return self.predict_t(treatment_t, covariate_t).cpu().data.numpy()
...
############ from DFIV.trainerATE import DFIVTrainer
...
def build_net(t_input_dim, v_input_dim, x_input_dim):
    ...
    instrumental_net = nn.Sequential(nn.Linear(v_input_dim+x_input_dim, 128),
                                     nn.ReLU(),
                                     nn.Linear(128, 64),
                                     nn.ReLU(),
                                     nn.Linear(64, 32),
...
def run(exp, data, train_dict, log, device, resultDir, others):
    ...
    t_input_dim, v_input_dim, x_input_dim = [train_dict['t_dim'], train_dict['z_dim'], train_dict['x_dim']]
    ...
    train_data = TrainDataSet(treatment=data.train.t,
                                instrumental=cat([data.train.z, data.train.x]),
                                covariate=data.train.x,
                                outcome=data.train.y,
                                structural=data.train.g)
    ...
    trainer = DFIVTrainer(train_dict['t_loss'], train_dict['y_loss'], logfile, _logfile, data_list, net_list, train_config, train_dict['GPU'], resultDir)
    test_loss, monitor, mdl = trainer.train(rand_seed=train_dict['seed'], verbose=1, epoch_show=5)

    def estimation(data):
        return mdl.predict(data.t-data.t, data.x, device),mdl.predict(data.t, data.x, device)
