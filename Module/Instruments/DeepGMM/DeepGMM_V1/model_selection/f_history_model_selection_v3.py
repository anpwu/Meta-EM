...
class FHistoryModelSelectionV3(object):
    ...
    def do_model_selection(self, x_train, z_train, y_train,
        ...
        for i, (g, f, learning_args) in enumerate(g_f_args_list):
            if verbose:
                logger.info(f"starting learning args eval %d" % i)
            g.initialize()
            f.initialize()
        ...
        for i, (g, f, learning_args) in enumerate(g_f_args_list):
            ...
            if verbose:
                logger.info(f"learning eval: {learning_eval}")
                logger.info(learning_args["game_objective"])
                logger.info(learning_args["g_optimizer_factory"])
                logger.info(learning_args["f_optimizer_factory"])
        ...
        if verbose:
            logger.info(f"size of f_z collection: {len(f_of_z_dev_list)}", )
