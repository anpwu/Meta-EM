...
class Gen_fn_IVCluster(object):
    def __init__(self, gens_dict, G=False):
        ...
        self.reps = gens_dict['reps']
        self.seed = gens_dict['seed']
        self.fn = gens_dict['fn']
        self.dataDir = gens_dict['dataDir']
        self.numDomain = gens_dict['numDomain']
        self.x_dim = gens_dict['x_dim']
        self.u_coef = gens_dict['u_coef']
        self.x_fn = gens_dict['x_fn']
        self.y_fn = gens_dict['y_fn']
        self.x4u = gens_dict['x4u']
        ...
        if not os.path.exists(self.dataDir + '/1/train.csv') or G:
            ...
            for rep_i in range(gens_dict['reps']):
                self.mean = None
                self.std  = None
