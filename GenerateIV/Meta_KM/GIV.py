...
def get_cluster(clusterKM, label):
    label = label.reshape(-1)
    cluster = copy.deepcopy(clusterKM.reshape(-1))
...
def generate_IV(data, train_dict, whichX='X'):
    ...
    kmeans.fit(input)
    cluster_kmeans = kmeans.predict(input)
    _, accuracy_kmeans = get_cluster(cluster_kmeans, label)

    print("Accuracy: {:.2f}%. ".format(accuracy_kmeans*100))

    Z = cluster_kmeans.reshape(-1,1)
    
    return Z, accuracy_kmeans

def process(args, Gen, Trainer, resultDir, whichX='newnewX', subDir='kmiv'):
    ...
    for D in [2,3,5,10]:
        ...
        print(f'KM({D})-Ind: ', Indepences[f'{D}'])
    ...
    os.makedirs(os.path.dirname(resultDir + f'Eval/KMIV/'), exist_ok=True)
    df.to_csv(resultDir + f'Eval/KMIV/{whichX}_IVind.csv')
...
def get_IV(data, resultDir, exp, D='best', whichX='newnewX', subDir='kmiv'):
    savepath = resultDir + f'{subDir}_{whichX}_{D}/'
